...
function SmartWizard(target, options) {
    ...
    var _loadContent = function ($this, stepIdx) {
        ...
        switch (stepIdx) {
            ...
            case 3:
                ...
                $.each($('input:radio').serializeArray(), function (key, value) {
                    out = out + "&" + value.name + "=" + value.value;
                });
                break;
        }
    ...
    SmartWizard.prototype.goForward = function ($this) {
        $this.options.contentCache = false;
        var value = validateSteps($this.curStepIdx + 1);
        if (value){
    ...
    function validateSteps(stepnumber){
        ...
        if(stepnumber == 2){
            var val = $('input[name=truckNumber]:checked').val();
            if(typeof(val)  === "undefined"){
                Materialize.toast("Select truck or change order", 4000);
                isStepValid=false;
            }
        }
        if(stepnumber == 3){
    ...
    SmartWizard.prototype.goBackward = function ($this) {
        $this.options.contentCache = true;
        var nextStepIdx = $this.curStepIdx - 1;
        if (0 > nextStepIdx) {
            if (!$this.options.cycleSteps) {
                return false;
